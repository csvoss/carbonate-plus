    ...
    if start_atom.is_chiral:
        ...
        if has_hydrogen:
            ...
            if has_parent:
                ...
                to_add = l
                if None in to_add:
                    to_add = [i if (i is not None) else Atom("H") for i in to_add]
                    ## TODO: This error happens sometimes???
                    ## raise StandardError("%s is chiral, but has two hydrogens." % start_atom.element)
                sort_atoms(to_add)
                for atom in to_add:
